...
model = Network(args, num_features=2048).to(device)
...
plt.title('Accuracy for ' + 'LDL trained ANN' + ' with ' + args.usenet + ' architecture')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
...
def visualize_filters(model, weight_name, max_layer):
    ...
    for i, m in enumerate(model_weights):
        ...
        for j in range(num_filters):
            ...
            plt.axis('off')
        plt.suptitle(
            f'Filters of LDL trained ANN with {args.usenet} architecture',
            fontsize=16)
