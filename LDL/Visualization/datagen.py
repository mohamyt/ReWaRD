...
def create_labeled_lmdb(src_lmdb, dst_lmdb, labels, map_size=None, safety_factor=1.2, min_map_size=(15 << 30)):
	...
	# If map_size not provided, estimate by summing sizes of values in source LMDB and add 4 bytes per entry for label
	if map_size is None:
		total_bytes = 0
		with lmdb.open(src_lmdb, readonly=True, lock=False, readahead=False) as env:
			with env.begin() as txn:
		...
		# add 4 bytes per entry for the label
...
def main(num_clusters=0, top_k=0):
	...
	# Dataset loader from LMDB
	if args.lmdb:
		from DataLoaderLMDB import Dataset_
		dataset = Dataset_(args.path2traindb, transform=_transform, train_portion=1.0, shuffle=False, val=True, seed=args.seed)
	else:
		from torchvision.datasets import ImageFolder
		dataset = ImageFolder(args.path2traindb, transform=_transform)
	...
	# If num_clusters==0, auto-select using silhouette score on a subset over candidate values
	if num_clusters == 0:
		...
		if best_k is None:
			num_clusters = candidates[0]
			print(f"Fallback num_clusters={num_clusters}")
		else:
			num_clusters = best_k
			print(f"Selected num_clusters={num_clusters} (silhouette={best_score:.4f})")
	...
	# optional: save pseudo-labels h5 (hard labels + top-k indices and weights)
	pseudo_h5 = os.path.join('data', 'pseudo_labels_ldl.h5')
	os.makedirs('data', exist_ok=True)
	print(f"Computing top-{top_k} pseudo-labels and saving to {pseudo_h5} (this may take a while)")
	...
	with h5py.File(pseudo_h5, 'w') as f:
		...
		if top_k > 0:
			# store top_k indices and weights to avoid full dense matrix
			topk_idx_ds = f.create_dataset('pseudo_topk_indices', shape=(num_samples, top_k), dtype=np.int32)
			topk_w_ds = f.create_dataset('pseudo_topk_weights', shape=(num_samples, top_k), dtype=np.float32)

			batch_size = 10000
			for i in tqdm(range(0, num_samples, batch_size), desc='Assigning top-k pseudo-labels'):
				...
				# get distances for those indices
				top_k_distances = np.take_along_axis(distances, top_k_indices, axis=1)
				top_k_weights = softmax(-top_k_distances, axis=1)
				topk_idx_ds[i:end, :] = top_k_indices.astype(np.int32)
				topk_w_ds[i:end, :] = top_k_weights.astype(np.float32)

	print(f"Saved pseudo labels to {pseudo_h5}")

	# GENERATE UDP MAP (UMAP) and save visualization
	print("Generating UDP map with UMAP...")

	# Subsample for UMAP if dataset too large
	...
	print(f" UDP (UMAP) map saved to 'data/udp_map.npy'")

	# compute silhouette score on a subset
	sil_score = None
	try:
		sil_sample_size = min(10000, num_samples)
		sil_idx = np.random.choice(num_samples, sil_sample_size, replace=False)
		sil_score = silhouette_score(latent_vectors[sil_idx], cluster_labels[sil_idx])
	except Exception:
		sil_score = None
	...
	# 3) create labeled LMDB
	src = args.path2traindb
	#dst = os.path.join(os.path.dirname(src), os.path.splitext(os.path.basename(src))[0] + '_clustered.lmdb')
	dst = './data/rwave-1024/rwave-1024_labelled.lmdb'
	dst = os.path.abspath(dst)
	print(f"Creating labeled LMDB at {dst}")
